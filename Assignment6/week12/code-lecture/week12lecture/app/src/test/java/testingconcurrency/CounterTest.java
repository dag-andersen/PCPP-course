...
public class CounterTest {
	...
	public void testingCounterParallel(int nrThreads, int N) {
		System.out.printf("Parallel counter tests with %d threads and %d iterations", nrThreads, N);
	...
	public void testingCounterParallelConstant() {
		...
		// assertTrue(N*2 == count.get(), "count.get() == "+count.get()+", but we
		// expected "+N*2);
	...
	public class Turnstile extends Thread {
		...
		public void run() {
			try {
				barrier.await();
				IntStream.range(0, N).forEach(x -> count.inc());
				barrier.await();
			} catch (InterruptedException | BrokenBarrierException e) {
				e.printStackTrace();
			}
