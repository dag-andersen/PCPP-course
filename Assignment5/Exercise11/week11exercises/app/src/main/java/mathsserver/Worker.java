...
public class Worker extends AbstractBehavior<Worker.WorkerCommand> {
    ...
    /* --- State ---------------------------------------- */
    private ActorRef<Server.ServerCommand> server;

    /* --- Constructor ---------------------------------- */
    private Worker(ActorContext<WorkerCommand> context, ActorRef<Server.ServerCommand> server) {
        super(context);
        this.server = server;
    }

    /* --- Actor initial state -------------------------- */
    public static Behavior<WorkerCommand> create(ActorRef<Server.ServerCommand> server) {
    ...
    public Receive<WorkerCommand> createReceive() {
        return newReceiveBuilder().onMessage(ComputeTask.class, this::onComputeTask).onMessage(Stop.class, this::onStop)
                .build();
    }

    /* --- Handlers ------------------------------------- */
    public Behavior<WorkerCommand> onComputeTask(ComputeTask msg) {
        getContext().getLog().info("{}: computing task {}", getContext().getSelf().path().name(), msg.task);
    ...
    private int taskExecutor(Task t) {
        int returnValue = 0;
        if (t.op == BinaryOperation.SUM) {
            returnValue = Sum(t.x, t.y);
        } else if (t.op == BinaryOperation.SUB) {
            returnValue = Sub(t.x, t.y);
        } else if (t.op == BinaryOperation.MUL) {
            returnValue = Mul(t.x, t.y);
        } else if (t.op == BinaryOperation.DIV) {
            returnValue = Div(t.x, t.y);
        }
        return returnValue;
    }
