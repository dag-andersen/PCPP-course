...
public class Client extends AbstractBehavior<Client.ClientCommand> {
	...
	/* --- Constructor ---------------------------------- */
	private Client(ActorContext<ClientCommand> context, ActorRef<Server.ServerCommand> server) {
		super(context);
		this.server = server;
	...
	public Receive<ClientCommand> createReceive() {
		return newReceiveBuilder().onMessage(ClientStart.class, this::onClientStart)
				.onMessage(TaskResult.class, this::onTaskResult).build();
	}
	...
	public Behavior<ClientCommand> onClientStart(ClientStart msg) {
		...
		getContext().getLog().info("{} started.", getContext().getSelf().path().name());
		getContext().getLog().info(tasks.toString());
		return this;
	}

	public Behavior<ClientCommand> onTaskResult(TaskResult msg) {
		getContext().getLog().info("{}: Result of task {} is {}", getContext().getSelf().path().name(), msg.task,
				msg.result);
		return this;
	}
}
