...
public class Observer extends AbstractBehavior<Observer.ObserverCommand> {
	...
	/* --- Constructor ---------------------------------- */
	private Observer(ActorContext<ObserverCommand> context, ActorRef<Observable.ObservableCommand> observable_actor) {
		super(context);
		this.observable_actor = observable_actor;
	}

	/* --- Actor initial behavior ----------------------- */
	public static Behavior<ObserverCommand> create(ActorRef<Observable.ObservableCommand> observable_actor) {
	...
	public Receive<ObserverCommand> createReceive() {
		return newReceiveBuilder().onMessage(BroadcastMessage.class, this::onBroadcastMessage)
			.onMessage(SubscriptionOK.class, this::onSubscriptionOK)
			.onMessage(UnsubscriptionOK.class, this::onUnsubscriptionOK)
			.onMessage(ObserverStarter.class, this::onObserverStarter).build();
	}

	/* --- Handlers ------------------------------------- */
	public Behavior<ObserverCommand> onBroadcastMessage(BroadcastMessage msg) {
		getContext().getLog().info("{}: Message '{}' by {} received", getContext().getSelf().path().name(), msg.message,
				msg.ref.path().name());
		return this;
	}
	...
	public Behavior<ObserverCommand> onSubscriptionOK(SubscriptionOK msg) {
		...
		if (new Random().nextBoolean()) {
			...
			getContext().getLog().info("{}: send message '{}'", getContext().getSelf().path().name(), message);
	...
	public Behavior<ObserverCommand> onUnsubscriptionOK(UnsubscriptionOK msg) {
		getContext().getLog().info("{}: Unsubcription confirmed by observable", getContext().getSelf().path().name());
		return this;
	}
