...
public class Guardian extends AbstractBehavior<Guardian.KickOff> {
	...
	public Behavior<KickOff> onKickOff(KickOff msg) {
		...
		final ActorRef<Observable.ObservableCommand> observableServer = getContext().spawn(Observable.create(),
				"observable_server");

		// spawn the N observers, and tell them to start
		final int N = 4;
		IntStream.range(1, N + 1).forEach((id) -> {
			final ActorRef<Observer.ObserverCommand> observer = getContext().spawn(Observer.create(observableServer),
					"observer_" + id);
